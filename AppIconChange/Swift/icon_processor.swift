...
func image(withImage image:NSImage, buildVersion:String, buildNumber:String, buildType:String) -> NSImage {
    ...
    let bluredImage = image.applyBlurWithRadius(blurRadius: size.width * 0.1, tintColor: NSColor(white: 0.11, alpha: 0.3), saturationDeltaFactor: 1.8)
    ...
    context.saveGState()
    context.clip(to: overlayFrame)
    bluredImage?.draw(in: bounds)
    context.restoreGState()
    
    //Calculate color and fonts and padding
    let textColor = image.averageColor()
    let typeFont = NSFont.systemFont(ofSize: overlayHeight * 0.8)
    let versionFont = NSFont.systemFont(ofSize: overlayHeight * 0.45)
    ...
    nsBuildType.draw(in: buildTypeFrame, withAttributes: buildTypeTextAttributes)
    ...
    nsBuildVersion.draw(in: buildVersionFrame, withAttributes: buildVersionTextAttributes)
    ...
    nsBuildNumber.draw(in: buildNumberFrame, withAttributes: buildVersionTextAttributes)
...
public extension NSImage {
    public func saveAsPNGatPath(path:String, atomically: Bool = true) -> Bool {
        let data = self.tiffRepresentation(using: NSTIFFCompression.none, factor: 1.0)!
...
//        if let imagePGNData: NSData = bitmap.representationUsingType(NSBitmapImageFileType.NSPNGFileType, properties: [NSImageCompressionFactor: 1.0]) {
        if let imagePGNData: Data = bitmap.representation(using: NSBitmapImageFileType.PNG, properties: [NSImageCompressionFactor: 1.0]) {
            return imagePGNData.write(to: NSString(string: path).standardizingPath, options: Data.WritingOptions.atomicWrite)
        } else {
            return false
        }
    }
